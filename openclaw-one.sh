...
def spawn_service(port=None):
    """Detach a persistent 'serve' process so the web keeps running after the
    installer/interactive session exits. Returns True if spawned."""
    if os.getenv("OPENCLAW_WEB_DISABLE","")=="1": return False
    p=str(port or int(os.getenv("OPENCLAW_WEB_PORT","8765")))
    script=os.path.abspath(__file__)
    try:
        env=dict(os.environ); env["OPENCLAW_HOME"]=str(ROOT); env["OPENCLAW_WEB_PORT"]=p
        log=ROOT.parent/"openclaw-serve.log"
        with open(str(log),"a",encoding="utf-8") as f:
            f.write("\n=== OpenClaw serve started %s ===\n"%time.strftime("%Y-%m-%d %H:%M:%S"))
        flog=open(str(log),"a",encoding="utf-8")
        subprocess.Popen(["/bin/bash", script, "serve"], env=env, stdout=flog,
                         stderr=subprocess.STDOUT, start_new_session=True, close_fds=True)
        log_event("service_spawned",port=p,pid_hint=script)
        return True
    except Exception as e:
        log_event("service_spawn_failed",error=str(e)); return False

def open_web(port=None):
    url="http://127.0.0.1:%s/"%(port or os.getenv("OPENCLAW_WEB_PORT","8765"))
    try:
...
def _build_start_app(desktop, bundle):
    ...
    try:
        ...
        # Detach serve with nohup so the web keeps running even if the Terminal
        # window closes, then open the browser automatically.
        logfile = bundle / "openclaw-serve.log"
        port = os.getenv("OPENCLAW_WEB_PORT", "8765")
        launch.write_text(
            "#!/bin/bash\n"
            "# OpenClaw launcher (self-contained, generated by install)\n"
            "# Starts the web Control Center in the background and opens the browser.\n"
            f'export OPENCLAW_HOME="{data}"\n'
            f'cd "{bundle}"\n'
            f'nohup "{script}" serve >> "{logfile}" 2>&1 &\n'
...
def main():
    ...
    if x.cmd=="memory":
        ...
        if spawn_app_serve(port):
            ...
            try:
                started=run_all(port)
                if any(k=="web" for k,_ in started):
                    print(f"[ web ] Control Center at http://127.0.0.1:{port}  (set OPENCLAW_WEB_DISABLE=1 to skip)")
        ...
        if not sys.stdin.isatty():
            print("[ services ] no TTY detected; service continues in the background (Ctrl-C to stop this launcher).")
            try:
                while True: time.sleep(3600)
            except KeyboardInterrupt: pass
        else:
